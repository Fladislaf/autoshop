...
let currentFilters = {
    boltPatterns: [], diameters: [], widths: [], offsets: [], dias: [], brands: [],
    sortOrder: 'cheap', searchQuery: null
};

async function initDiskiPage() {
    ...
    currentFilters.boltPatterns = urlParams.get('bolt') ? urlParams.get('bolt').split(',') : [];
    ...
    currentFilters.offsets = urlParams.get('et') ? urlParams.get('et').split(',') : [];
    currentFilters.dias = urlParams.get('cb') ? urlParams.get('cb').split(',') : [];
    currentFilters.brands = urlParams.get('brand') ? urlParams.get('brand').split(',') : [];


    const availableFilters = await API.getWheelFilters();
    if (availableFilters) {
    ...
    if (resetBtn) {
        resetBtn.addEventListener('click', () => {
            ...
            window.location.href = `diski.html?${params.toString()}`;
        });
    }
...
function renderDynamicFilters(data) {
    const createCheckboxes = (containerId, items, nameAttr, selectedArr) => {
        ...
        if (items.length === 0) {
            container.innerHTML = '<span style="color:#999; font-size:13px; margin-left: 10px;">Нет вариантов</span>';
            return;
        }
    ...
    createCheckboxes('filterBoltList', data.boltPatterns, 'bolt', currentFilters.boltPatterns);
    ...
    createCheckboxes('filterEtList', data.offsets, 'et', currentFilters.offsets);
    createCheckboxes('filterCbList', data.dias, 'cb', currentFilters.dias);
...
async function loadCatalogPage(page) {
    ...
    grid.innerHTML = '<div style="grid-column: 1/-1; text-align: center; padding: 50px; color: #888; font-size: 16px;">⏳ Загрузка дисков...</div>';
    ...
    const response = await API.getWheels(currentFilters, currentFilters.sortOrder, currentPage, ITEMS_PER_PAGE);
    const wheels = response.items;
    ...
    if (currentFilters.searchQuery) {
        ...
        if (breadcrumbs) breadcrumbs.innerHTML = `<a href="diski.html" style="color: #888; text-decoration: none;">Шины и диски</a> <span style="margin: 0 5px;">/</span> Поиск`;
    } else {
        if (h1) h1.textContent = `Диски`;
        if (breadcrumbs) breadcrumbs.innerHTML = `Шины и диски <span style="margin: 0 5px;">/</span> Диски`;
    }

    grid.innerHTML = '';
    appendWheelsToGrid(wheels);
    renderPagination();
}
...
function appendWheelsToGrid(wheels) {
    const grid = document.querySelector('.catalog-grid');
    if (!grid) return;

    if (wheels.length === 0 && currentPage === 1) {
        grid.innerHTML = '<p style="padding: 20px; grid-column: 1/-1;">Диски по заданным параметрам не найдены.</p>';
        return;
    }
    ...
    wheels.forEach(item => {
        ...
        const cardHTML = `
            <a href="product.html?id=${item.id}&type=wheel" class="product-card">
...
document.addEventListener('DOMContentLoaded', initDiskiPage);